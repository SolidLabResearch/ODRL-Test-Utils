...
async function rdf2store(str) {
    return new Promise( (resolve) => {
        const parser = new N3.Parser();
        const store = new N3.Store();

        parser.parse(str, (error, quad, _) => {
            if (error) 
...
async function asJSONLD(str) {
    return new Promise( async (resolve) =>  {
        const store = await rdf2store(str);
        ...
        resolve(await stringifyStream(textStream));
    });
}
